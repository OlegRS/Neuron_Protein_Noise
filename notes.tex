...
\begin{equation*}
  \begin{split}
    & \frac{\partial\mathcal G(\mathbf x; t)}{\partial t} + \left(\lambda^+_1x_n^2-\lambda_2x_{m_0}x_n + \frac{1}{\tau_1}x_n - \lambda_2x_{m_0}\right)\frac{\partial\mathcal G(\mathbf x; t)}{\partial x_n}\\
    &-\left[\kappa_0x_{m_0}x_{l_0} + \kappa_0x_{l_0} + \nu_{01}x_{m_1} - \left(\frac{1}{\tau_2}+\nu_{01}\right)x_{m_0}\right]\frac{\partial \mathcal G(\mathbf x; t)}{\partial x_{m_0}}\\
    &-\left[\kappa_1x_{m_1}x_{l_1} + \kappa_1x_{l_1} + \nu_{10}x_{m_0} - \left(\nu_{10}+\nu_{12}+\nu_{13}+\frac{1}{\tau_2}\right)x_{m_1} + \nu_{12}x_{m_2} + \nu_{13}x_{m_3}\right]\frac{\partial\mathcal G(\mathbf x; t)}{\partial x_{m_1}}\\
    ...
    &-\left[\eta_{11}x_{q_{11}}+\eta_{12}x_{q_{12}}+\theta_{10}x_{l_0} - \left(\eta_{11}+\eta_{12}+\theta_{10}+\theta_{12}+\theta_{13}+\frac{1}{\tau_3}\right)x_{l_1} + \theta_{12}x_{l_2} + \theta_{13}x_{l_3}\right]\frac{\partial\mathcal G(\mathbf x; t)}{\partial x_{l_1}}\\
    &-\left[\eta_{21}x_{q_{21}}+\eta_{22}x_{q_{22}}+\theta_{21}x_{l_1}-x_{l_2}\left(\eta_{21}+\eta_{22}+\theta_{21}+\frac{1}{\tau_3}\right)\right]\frac{\partial\mathcal G(\mathbf x;t)}{\partial x_{l_2}}\\ 
    &-\left[\eta_{31}x_{q_{31}}+\eta_{32}x_{q_{32}}+\theta_{31}x_{l_1}-x_{l_3}\left(\eta_{31}+\eta_{32}+\theta_{31}+\frac{1}{\tau_3}\right)\right]\frac{\partial\mathcal G(\mathbf x;t)}{\partial x_{l_3}}\\ 
...
\begin{table}
  \begin{tabular}{ |c|c|c|c| }
    \hline
    Variable &Expectation & RMS    & RMS/Expectation\%\\  \hline
    $n$      & $1$                 & $0$ & $0$ \\                 \hline
    ...
    \hline
  \end{tabular}
  \caption{\label{table:expectations_errors}Expectations, noise and noise to signal ratios.}
\end{table}
...
In this section we give the general rules for constructing a {\it master equation} from the neuron's compartments diagram analogous to the one in Figure \ref{fig:simplest_neuron}.
\subsubsection{Soma}
Soma is always indexed as $0$.

Parameters: $\{N$, $\lambda_1^+$, $\lambda_1^-$, ($\tau_1=(\lambda_1^+ + \lambda_1^-)^{-1}$), $\lambda_2$, $\tau_2$, $\kappa_0$, $\tau_3\}$.


Additional terms in the {\it master equation}:
\begin{equation}
  \begin{split}
...
\subsubsection{Dendrite $i$ from the soma}
Additional terms in the {\it master equation}:
\begin{equation}
  \begin{split}
    & \textcolor{green}{\frac{1}{\tau_2}\left[(m_i+1)P_{m_i+1}^{\mathbf s} - m_iP_{\mathbf s}\right]} \textcolor{blue}{+\nu_{0i}\left[(m_0+1)P_{m_0+1, m_i-1}^{\mathbf s}-m_0P_{\mathbf s}\right]}\\
    &\textcolor{blue}{+\nu_{i0}\left[(m_i+1)P_{m_0-1,m_i+1}^{\mathbf s} - m_iP_{\mathbf s}\right]}
  \end{split}
\end{equation}


\subsubsection{Branching of dendritic branch $i$ to branches $j$ and $k$}
\subsubsection{Synapse $j$ on dendrite $i$}

%% \subsection{The simple model of neuron}
%% Now we consider a model of neuron with a branched dendrite and six synapses. Similarly to the case of the simplest neuron model above, the model should capture the following processes:
%% \begin{enumerate}
%% \item \textcolor{red}{Gene activation/deactivation};
%% \item \textcolor{green}{mRNA production (only in the soma) and degradation (in all compartments)};
%% \item \textcolor{blue}{mRNA hopping};
%% \item \textcolor{brown}{Free protein production (in all compartments) and degradation (in all compartments, but not in the synapses)};
%% \item \textcolor{cyan}{Free protein hopping};
%% \item \textcolor{pink}{Protein binding/unbinding from the synapses}.
%% \end{enumerate}
