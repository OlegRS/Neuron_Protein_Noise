...
num = np.genfromtxt("../../data/no_dendrites_slow_mRNAs", delimiter=',')[:x_lim, :]
...
for file_id in range(sim_run_count):
    file_name = "../../data/gillespie/test/no_dendrites/slow_mRNAs/g_" + str(file_id)
    print(file_name)
...
axs2[1].plot(variances[:,0], variances[:,2], linestyle='--', color='pink')
axs2[1].plot(num[:,0], num[:,4], linestyle='--', color='red')
axs2[1].grid()

axs2[2].plot(variances[:,0], variances[:,3], linestyle='--', color='pink')
axs2[2].plot(num[:,0], num[:,6], linestyle='--', color='red')
axs2[2].grid()
