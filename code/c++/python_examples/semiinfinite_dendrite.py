...
P_discrete = np.genfromtxt("protein_expectations.csv", delimiter=',')
...
############## mRNA-mRNA ##############
mRNA_covariances = np.genfromtxt("mRNA_mRNA_covariances.csv", delimiter=',')
...
for i in range(PCCs.shape[0]):
    for j in range(i):
        PCCs[i,j] = PCCs[j,i] = (mRNA_covariances[i,j] - mRNA_expectations[i]*mRNA_expectations[j])/(mRNA_stds[i]*mRNA_stds[j])
...
######### mRNA-prot ###########
prot_covariances = np.genfromtxt("prot_prot_covariances.csv", delimiter=',')
...
for i in range(PCCs.shape[0]):
    for j in range(i):
        PCCs[i,j] = PCCs[j,i] = (mRNA_prot_covariances[i,j] - mRNA_expectations[i]*prot_expectations[j])/(mRNA_stds[i]*prot_stds[j])
...
for i in range(PCCs.shape[0]):
    for j in range(i):
        PCCs[i,j] = PCCs[j,i] = (prot_covariances[i,j] - prot_expectations[i]*prot_expectations[j])/(prot_stds[i]*prot_stds[j])
