...
\begin{multline}\label{nonstat_expectations_result}
  ...
  = \lambda_1^+N\sum_k\lambda_k^{-1}S_{ik}\mathbf S^{-1}_{k1} + \sum_{k=1}^{d_1}\mathrm e^{-\lambda_kt}S_{ik}\sum_{l=1}^{d_1}\mathbf S^{-1}_{kl}c_l,
\end{multline}
from where
\begin{equation}
...
\begin{multline} \label{nonstationary_o2_solution}
  ...
  = \sum_{j=1}^{d_2}T_{ij}\left\{\mathrm e^{-\lambda^{(2)}_jt}\sum_{s=1}^{d_2}\mathbf T^{-1}_{js}\mathcal G^{(2)}_s(t=0) + \sum_{k=1}^{d_2}\mathbf T^{-1}_{jk}\sum_{\eta=1}^{d_1}D_{k\eta}\int_0^t\mathrm e^{-\lambda^{(2)}_j(t-\tau)}\mathcal G^{(1)}_\eta(\tau)d\tau\right\}.
\end{multline}
