...
This section is concerned with correlations of the observables $\boldsymbol\Omega$ shifted in time by $\tau$ at stationarity, i.e. $\lim_{t\to\infty}\langle\boldsymbol{\Omega}(t)\otimes\boldsymbol{\Omega}(t+\tau)\rangle$.

It can be shown that
\begin{equation}
  \lim_{t\to\infty}\langle\boldsymbol{\Omega}(t)\otimes\boldsymbol{\Omega}(t+\tau)\rangle = (1-\mathrm e^{-\mathbf A\tau})\langle\boldsymbol\Omega\rangle(\infty)\otimes\langle\boldsymbol\Omega\rangle(\infty) + \mathrm e^{-\mathbf A\tau}\langle\boldsymbol\Omega\otimes\boldsymbol\Omega\rangle(\infty),
\end{equation}
where $\mathbf A$ is the matrix of first-order dynamics from Section \ref{subsec:o1_nonstat_problem}, and, as discussed earlier, $\langle\boldsymbol\Omega\rangle = \boldsymbol{\mathcal G}^{(1)}$, while $\langle\boldsymbol\Omega\otimes\boldsymbol\Omega\rangle$ can be found as $\langle\Omega_i\Omega_j\rangle = {\mathcal G}^{(2)}_{ij} + \mathcal G^{(1)}_i\delta_{ij}$.
%% \begin{equation}
%%   \begin{dcases}
%%     \langle\Omega_i\Omega_j\rangle = {\mathcal G}^{(2)}_{i,j}, & i\neq j\\
%%     \langle\Omega_i\Omega_j\rangle = {\mathcal G}^{(2)}_{i,j} + {\mathcal G}^{(1)}_i, & i=j\\
%%   \end{dcases}
%% \end{equation}
